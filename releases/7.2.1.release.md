...
* Added option to `ky` to completely eliminate the automatic position requests that Pegasus makes as part of trips detection
* Exposed a way to get the amount of triggers per entities
* Supported 4 videos for CMS DVR
* Added `ftgfoodrnk` label for Food and Drink events in Surfisght retransmitter
* Added Cipia retransmitter for the Australia middleware
* Supported speed,heading information and low battery event for Satcom lite devices
...
* Change `engine.ignition.status` depending on whether the label is ignon/ignoff for Jimi devices
* Updated the labels for Surfsight to generate ignition events
